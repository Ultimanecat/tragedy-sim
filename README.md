...
主人公玩家；房主可用随机 AI 填充任意空位，主人公席位可选择“基础干扰”“公开信息防守”、实验性的“历史风险”或
“ISMCTS（FS/BTX）”AI，剧作家席位可选择定式 AI、朴素 MCTS、优化 MCTS 或策略 MCTS。基础干扰 AI 会在下一天尝试
...
真实暗牌。大厅中的 `ismcts_protagonist` 会在主人公出牌阶段从这些候选世界重建公开棋盘，并在根信息集聚合行动访问、收益和
availability；能力与事件内部选择尚不能精确重建时，明确回退到公开信息防守策略。服务端仍未为命令建立完整的可见性标记，
因此跨阶段树和房间级持续粒子尚未完成。
引擎现已在每次真实决策后保存各主人公语义快照的短摘要，搜索模拟不会写入该记录；后续粒子过滤可以仅凭这些摘要推进，
而无需读取完整命令历史。粒子过滤器会保留所有观测相容解释、按私密状态去重，并用固定种子的蓄水池采样限制宽分支；
房间级增量维护和命令可见性标记仍在开发中。FS/BTX 的硬 witness 会用公开身份、当事人、规则和已发生事件阈值排除矛盾
