...
这是支持真人、随机 AI、定式与完全信息 MCTS 剧作家 AI，以及 FS/BTX 第一版主人公 ISMCTS 的热座/局域网裁判工具，不含公网大厅。
...
1 名主人公玩家的两人局中还可选择“团队 ISMCTS（FS/BTX）”AI。剧作家席位可选择定式 AI、朴素 MCTS、优化 MCTS 或策略 MCTS。基础干扰 AI 会在下一天尝试
逆转剧作家的成功移动，并约定仅逻辑领队使用禁止密谋；防守版还会根据公开事件、身份、计数物和结构化好感能力选择行动；
历史风险版会按时间衰减统计公开揭牌中的剧作家关注目标。四种主人公 AI 均不读取真实剧本秘密。定式 AI 会从规则失败、
...
C3 的主人公信息集搜索已经提供 FS/BTX 第一版。`PublicEvidence` 已建立严格的信息边界；`ConstraintBeliefSampler` 不查询剧本 ID，
而是按规则 X/Y 身份槽、公开事件日程和已揭示事实生成经校验的隐藏世界。标准粒子已经可以物化并重放显式公开命令，
再用语言无关的公开快照过滤矛盾世界；对于未公开命令，会枚举粒子内的合法行动并保留所有公开现象相同的后继，不读取
真实暗牌。大厅中的 `ismcts_protagonist` 是两人局团队 AI：一个实例共享证据与粒子并控制 A/B/C。主人公阶段生成完整
三牌候选并跨粒子重复比较；选中组合后按引擎顺序依次提交。`ismcts_legacy_protagonist` 保留“第一张搜索、后两张由防守
策略补全”的旧版供对照。能力与事件内部选择尚不能精确重建时，明确回退到公开信息防守策略。
引擎在每次真实决策后保存语义快照短摘要，并把命令观察显式标为 `public / partial / hidden`；未翻开的剧作家牌只保留公开
目标，搜索必须枚举相容牌面。粒子集合会跨决策增量推进、按私密状态去重，并用固定种子的蓄水池限制宽分支。FS/BTX 的硬 witness 会用公开身份、当事人、规则和已发生事件阈值排除矛盾
setup；事件未发生和可选能力未发动不会被错误当成身份反证。详见 [ISMCTS 与 witness 证据设计](docs/ismcts-witness-design.md)。
