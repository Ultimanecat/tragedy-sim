...
大厅内置八个原创教学局，以及 22 张由项目维护者所提供卡图人工录入的 FS、BT/BTX、MZ、MC 官方剧本卡；
不同轮回数难度展开后共有 40 个官方可选条目。
版图位于手牌上方；行动牌可以直接拖到服务端标出的合法角色或版图，也始终可以使用“点击牌、点击版图目标、
确认执行”的操作方式。可发动角色能力时，先在版图点击带“可发动”标记的角色，再选择具体能力。
剧作家出牌采用三牌草稿：每张牌先放入草稿，可撤回、换牌、改目标或清空，三张齐全后一次确认提交。
三牌操作栏固定在屏幕底部，显示已选数量与当前编辑席位；选牌支持取消，无效拖放会给出提示，手机可随时回退与提交。
两人局中，一名主人公玩家控制 A/B/C 时也采用三牌草稿；三人和四人局红方仍逐张出牌。
草稿只保存在当前页面，确认前对手看不到；刷新或切换调试视角会丢弃草稿，普通同步和提交失败会保留。
如果服务端状态已改变，客户端会刷新并清除过期草稿，防止按旧局面误提交。
完整提交在后端原子验证，任何一张非法都不会留下部分放牌。已逐张放过牌的旧存档继续使用单步流程。
公开日志可以按今日、本轮回或全部范围筛选和搜索；完整对局回放以决策时间轴显示每次选择及其结算记录，
同时保留原始纯文本供核对和导出。
当前行动栏会提示出牌、能力选择、最终猜测和等待／同步操作。公开日志默认显示最近 12 条，支持只看关键记录，
以独立样式标注死亡、失败、能力拒绝、事件及信息获取；揭牌的完整牌组可展开查看，不删除底层记录。
版图角色和行动牌会显示由 `resources/data.xml` 映射、为浏览器单独生成的 WebP 缩略图；原始素材不会被修改。
单一区域达到 5 人时会自动改用紧凑网格，9 人以上使用高密度布局；角色资料通过独立浮层查看，不会撑乱版图。
若更新了 `resources/` 中的卡图或索引，请在 `web` 目录运行 `npm run assets`，再运行 `npm run build`。
各规则集的 Ex/诅咒/尸体/世界状态、LL 主人公秘密和 AHR 分面最终猜测均有独立展示；
法术、身份宣称及特殊事件选择仍由后端作为合法行动提供，不在前端重复实现规则。
...
先按上面的步骤执行一次 `npm run build`，然后在作为主机的电脑运行：
...
1 名主人公玩家的两人局中还可选择“团队 ISMCTS”“当日生存优先 ISMCTS”及旧版对照（均限 FS/BTX 完整搜索）、FS/BTX 的“粒子集成”实验 AI，以及 FS/BTX 的两个“已知剧本”诊断 AI（明牌/暗牌）。粒子集成版只读取主人公可见视图，把身份、事件当事人和暗牌假设分开采样，联合评估当天三张牌；BTX 最终猜测直接比较完整身份分配的 witness 权重。其日末估值还会从公共粒子熵估算尚可兑现的信息能力价值，但该微量奖励严格排在当日存活之后，详见 [信息价值设计](docs/information-value-design.md)。当前房间配置为秒级预算，仍不能完整评价跨轮回信息价值。剧作家席位可选择定式 AI、朴素 MCTS、优化 MCTS、策略 MCTS、FS/BTX 三牌联合搜索，以及 BTX 专用的实验性信念采样联合搜索。基础干扰 AI 会在下一天尝试
...
轻量搜索副本、渐进拓宽和行动先验；策略 MCTS 在此基础上让剧作家 rollout 沿同一剧本获胜路线连续行动。三者都使用可复现的节点预算和
剧作家视角局面评价；三牌联合版用公开信息策略模拟主人公回应。BTX 信念采样版从主人公可见证据抽取可能世界和联合三牌回应，用根节点 UCB 分配模拟；它仍缺少跨日树和最终猜测风险估值，首轮低预算测试也未显示稳定提升。大厅使用较小预算以保持交互速度，目前也不保证战胜真人反制。
...
手机与主机需连接同一个普通家庭 Wi-Fi。若手机打不开链接：

- 在 Windows 防火墙提示中允许 Python 使用“专用网络”，或为 TCP 8765 端口添加专用网络入站规则；
- 确认 Wi-Fi 不是“访客网络”，路由器没有启用 AP/客户端隔离；
- 保持终端窗口运行；当前房间保存在内存中，停止服务会关闭所有房间。

局域网模式不会配置端口转发，也不应直接暴露到互联网。房间码不是私密视图凭据；每台设备只保存自己的随机房间令牌，
剧作家、主人公和旁观者看到的内容由后端分别过滤。房主可以在等待大厅释放误占座位、关闭房间，并在游戏中保存或导出回放；
房间回放仍按 A/B/C 逻辑席位确定性重演，并附带实际执行者昵称供复盘。
...
- 新建各支持模组的教学局、载入自定义剧本、恢复/保存任意中间状态，以及打开只读回放。
...
| `guessall <领队> <角色=身份,...>` | 一次提交全部身份；公布正确数，全部正确才获胜 |
| `final <领队>` | 允许提前决战的模组中，在轮回之间放弃剩余轮回并开始猜测 |
| `save "session.json"` | 保存到新文件；已存在的文件不会覆盖；路径可加双引号 |
| `replay "finished.tlr"` | 正式结束后导出纯文本完整信息回放；不会覆盖已有文件 |
| `help` / `quit` | 帮助 / 退出 |

日常领队按 A → B → C 轮换；中途结束轮回不会额外轮换。
失败之后用 `next m` 开始下一轮。FS 轮回耗尽即判负；支持最终决战的模组会进入最终猜测。
...
Web 大厅会从 `GET /v1/scenarios` 读取可选剧本。仓库内置八个原创教学剧本和 22 张已核对的官方剧本卡；
卡面有多个轮回数时会展开成标准版、Easy 版（以及必要时 Very Easy 版），目前共 40 个官方可选条目。把新剧本 JSON
...
牌组依据：用户提供的 `resources/data.xml` 手牌索引、各规则集速查表和对应牌面。
