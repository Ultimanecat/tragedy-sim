...
1 名主人公玩家的两人局中还可选择“团队 ISMCTS”“当日生存优先 ISMCTS”及旧版对照（均限 FS/BTX 完整搜索）、FS/BTX 的“粒子集成”实验 AI，以及只适用于 FS 的两个“已知剧本”诊断 AI（明牌/暗牌）。粒子集成版只读取主人公可见视图，把身份、事件当事人和暗牌假设分开采样，联合评估当天三张牌；BTX 最终猜测直接比较完整身份分配的 witness 权重。当前房间配置为秒级预算，单日搜索还不能评价跨轮回信息价值。剧作家席位可选择定式 AI、朴素 MCTS、优化 MCTS、策略 MCTS，或 FS 专用的三牌联合搜索。基础干扰 AI 会在下一天尝试
