...
BTX 的 `mandatory_serial_route` 也使用硬析取：初始杀人狂，或“病毒副规则＋本轮已达不安 3 的初始普通人”。引擎公开的 `mandatory_window_resolved` 只标记日末强制能力已完成，不公开身份、来源或暗牌。组件只读取有完整边界的日志，并使用首个受攻击目标；后续强制批次可能受前一批死亡或转化影响，不能直接沿用开窗快照。旧 replay 缺少边界时返回未知。
