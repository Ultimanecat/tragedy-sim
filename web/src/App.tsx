...
export default function App() {
  ...
  async function setAiRoomSeat(seat: Seat, enabled: boolean,
                               strategy: "random" | "fixed_mastermind" | "mcts_mastermind" = "random") {
    setBusy(true); setError("");
      ...
      {!roomInfo ? <h2>正在连接房间 {roomCode}…</h2> : <>
        ...
        <div className="seat-grid">{roomInfo.room.required_seats.map(seat => {
          ...
          return <article className={`${occupant ? "occupied" : ""} ${occupant?.ai ? "ai-seat" : ""}`} key={seat}>
            ...
            <span>{occupant?.ai ? (occupant.ai_type === "fixed_mastermind" ? "从可行获胜定式中择一执行" : occupant.ai_type === "mcts_mastermind" ? "完全信息蒙特卡洛树搜索" : "自动随机行动") : occupant ? (occupant.ready ? "已准备" : "尚未准备") : "等待加入"}</span>
            ...
            {client.room?.adminToken && !occupant && seat === "m" && <button disabled={busy}
              onClick={() => void setAiRoomSeat(seat, true, "mcts_mastermind")}>MCTS 剧作家 AI</button>}
