...
class RoomService:
    ...
    def set_ai(self, code: str, request: Any, *, token: str | None) -> dict[str, Any]:
        ...
        protagonist_strategies = {"baseline_protagonist", "defensive_protagonist"}
        if strategy not in ("random", *mastermind_strategies, *protagonist_strategies):
            raise ServiceError(
                "INVALID_AI_STRATEGY",
                "AI 策略必须是 random、baseline_protagonist、defensive_protagonist、fixed_mastermind、mcts_mastermind、optimized_mcts_mastermind 或 strategic_mcts_mastermind")
