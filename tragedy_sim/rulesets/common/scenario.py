...
def validate_scenario(data: dict) -> dict:
    ...
    if not isinstance(data, dict) or set(data) - required - {'table_talk', 'wm_replacement_plot', 'hidden_cast', 'll_secret_order'} or required - set(data):
    ...
    if 'wm_mad_truth' in plots:
        if not isinstance(replacement, str) or replacement == data['main_plot'] or replacement not in spec.plots or (PLOTS[replacement][1] != 'Y'):
    ...
    if 'hsa_girl_crisis' in plots and any(('girl' not in CHARACTERS[c].traits for c, r in cast.items() if r == 'key')):
    ...
    for incident in data['incidents']:
        ...
        group_incident = False
        if not isinstance(culprit, str) or (culprit not in {'hospital', 'shrine', 'city', 'school'} if group_incident else culprit not in cast):
    ...
    if any((role == 'obsessive' for role in cast.values())) and (not any((cast[culprit] == 'obsessive' for culprit in culprit_kinds))):
        raise RuleError('强迫症必须担任至少一起事件的当事人')
    if any((role == 'detective' for role in cast.values())) and any((cast[culprit] == 'detective' for culprit in culprit_kinds)):
        raise RuleError('侦探不能担任事件的当事人')
    for role, name in (('fool', '愚者'), ('twin', '双胞胎')):
        if any((value == role for value in cast.values())) and (not any((cast[culprit] == role for culprit in culprit_kinds))):
